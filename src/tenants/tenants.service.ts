...
import { InviteResponseDto, MemberResponseDto, TenantResponseDto } from './dto/tenant.dto';
...
export class TenantsService {
  ...
  async getOne(userData: UserData): Promise<TenantResponseDto & { role: string }> {
    const [tenant, member] = await Promise.all([
      ...
      this.memberRepo.findOneOrFail({ where: { tenantId: userData.tenantId, userId: userData.id, deletedAt: IsNull() } })
    ])
  ...
  async inviteUser(dto: InviteDto, userData: UserData): Promise<InviteResponseDto> {
    const invite = await this.tenantHelperService.generateInvitationData(dto, userData)
  ...
  async getMembers(query: PaginateQuery, userData: UserData): Promise<{ meta: any, data: MemberResponseDto[] }> {
    ...
    const members = await this.tenantHelperService.getMembers(query, userData)
    return {
      ...members,
  ...
  async getMember(id: string, userData: UserData): Promise<MemberResponseDto> {
    const member = await this.tenantHelperService.getMember(id, userData)
  ...
  async updateMemberRole(id: string, dto: UpdateMemberDto, userData: UserData): Promise<MemberResponseDto> {
    const member = await this.tenantHelperService.updateMemberRole(id, dto, userData)
    return plainToInstance(MemberResponseDto, member, { excludeExtraneousValues: true })
  ...
  async removeMember(id: string, userData: UserData) {
    ...
    await this.tenantHelperService.removeMember(id, userData)
